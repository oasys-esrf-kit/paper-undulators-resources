...
def run_spectra(res_energy, shift, syned_json_file, harmonic=1, plot=False, zero_emittance=False, zero_spread=False):
    ...
    detuning = ((res_energy + shift) - res_energy) / res_energy
    print("For this case we have a detunign of {}".format(detuning)) 
    ...
    prm["Light Source"]["K value"] = e_to_k(res_energy, syned_json_file, harmonic=harmonic)    
    prm["Configurations"]["Target Harmonic"] = harmonic
    prm["Configurations"]["Detuning"] = detuning
    ...
    if plot:
        ...
        plt.title("{} Res energy {} shift {} eV".format(\
                    und_name, res_energy, shift), fontsize=f_size)
...
def test_save_h5(res_energy, shifts, syned_json_file, harmonic=1, spectra_data= 'charac_at_source_point_off_res', save_file=True, zero_emittance=False, zero_spread=False):
    
    """ Runs SPECTRA for a list of shifts, for a given harmonic,
    ...
    for shift in shifts:
        ...
        x, y, zdata = run_spectra(res_energy, shift, syned_json_file, harmonic=harmonic, zero_emittance=zero_emittance, zero_spread=zero_spread)
    ...
    if save_file:
        ...
        if len(shifts) == 1:
            #single energy
            HDF5_FILE = 'Spectra_{}_{}_shift_{}'.format(spectra_data, syned_obj.get_name(), shifts[0])
        else:
...
if __name__=='__main__':
    ...
    #run_spectra(10000, -400, "ESRF_ID06_EBS_CPMU18_1.json", harmonic=1, plot=True, zero_emittance=True, zero_spread=True)
    #shifts = numpy.arange(-400, 100, 1)
    shifts = numpy.arange(-1000, 1000, 2)
    #print(len(shifts))    
    test_save_h5(10000, shifts, "ESRF_ID06_EBS_CPMU18_1.json", harmonic=1, save_file=True, zero_emittance=True, zero_spread=True)

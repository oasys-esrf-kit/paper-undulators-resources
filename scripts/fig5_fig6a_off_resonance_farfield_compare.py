...
if __name__ == "__main__":
    ...
    if True:
        ...
        plot_image(YW_IMG, Shift, x * 1e3,
                   yrange=[-2, 2],
                   xtitle=r"Photon energy $E-E_0$ [eV]", ytitle="x [mm] @ far field (100m)", title="",
                   aspect='auto', figsize=(9,7), show=0)
        plt.savefig("detuning-map-resonance.png")
        ...
        plot(ShiftW, 1e3 * YW,
             ShiftW, 1e3 * SDW,
             xtitle=r"Photon energy $E-E_0$ [eV]", ytitle="width [mm] @ far field (100 m) [m]",
             legend=['FWHM','SD'],
             xrange=[-300,500], yrange=[0,5], figsize=(12,8), show=0)
        plt.savefig("angular_width_vs_detuning.pdf")
    ...
    if False: # and harmonic_number < 2:
        ...
        plot_image(Y_IMG, Shift, y_div * 1e-3 * 100,
                   yrange=[-0.002, 0.002],
                   xtitle="Shift [eV]", ytitle="Far field x[mm]", title="SPECTRA",
                   aspect='auto', figsize=(9,7), show=0)
        ...
        plot(Shift,  1e3 * YS,
             Shift,  1e3 * SD,
             xtitle=r"Photon energy $E-E_0$ [eV]", ytitle="width [mm] @ far field (100 m) [m]",
             legend=['FWHM (SPECTRA)','SD (SPECTRA)'],
             xrange=[-300,500], yrange=[0,5], figsize=(12,8), show=0)

    # size vs offset comparison
    if False:
        # plot(Shift, YS,
        #      Shift, SD * 2.355,
        ...
        plot(Delta,  1e3 * YS,
             ...
             Delta,  1e3 * SD,
             DeltaW, 1e3 * SDW,
             xtitle="Delta=N n DE/Eo", ytitle="width [mm] @ far field (100 m) [m]",
             legend=['FWHM (SPECTRA)','FWHM (WOFRY)','SD (SPECTRA)','SD (WOFRY)'],
             xrange=[-2,5], yrange=[0,5], show=0)
        ...
        plot(Shift,  1e3 * YS,
             ...
             xtitle=r"Photon energy $E-E_0$ [eV]", ytitle="width [mm] @ far field (100 m) [m]",
             legend=['FWHM (SPECTRA)','FWHM (WOFRY)','SD (SPECTRA)','SD (WOFRY)'],
             xrange=[-300,500], yrange=[0,5], show=0)
